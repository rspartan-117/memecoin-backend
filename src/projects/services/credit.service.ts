...
/**
 * Custom domain credit surcharge — included in every deployment.
 * All deployments require a custom domain.
 *
...
export const MEMEGPT_WEB_SEARCH_CREDIT_COST = 5;
...
export interface ToolChargeParams {
  ...
  toolName: string;
  creditsToDeduct: number;
  /** Event type for OpenMeter analytics:
   * - 'meme-gen-ai-model': Landing page generation tools/LLM
   * - 'meme-gpt': All MemeGPT charges (tools + future LLM token tracking)
   */
  toolType?: 'meme-gen-ai-model' | 'meme-gpt';
}
...
export class CreditService {
   ...
   *
   * Cost: DEPLOYMENT_CREDIT_COST + CUSTOM_DOMAIN_CREDIT_SURCHARGE
   *       (1,100 + 100 = 1,200 credits = $6.00) per trigger.
   * Covers: 1 month of Koyeb nano hosting ($2.68) + DO Spaces storage (~$0.01)
   *         + SaaS Custom Domains ($0.20) at 2× profit margin.
   *
   * Refund policy:
   *   - Pre-queue failure (deployment record or queue enqueue fails before job exists):
   *     full refund via refundDeploymentCredits() — no cloud resources were allocated.
   *   - Post-queue failure (actual deployment job fails mid-run): NO refund —
   *     Koyeb/GitHub resources are consumed regardless.
   *
  ...
  async chargeForDeployment(
    ...
    this.sendToOpenMeter({
      ...
      credits: creditsToDeduct,
      type: 'meme-gen-deployment', // reuse existing event type for analytics
      metadata: {
  ...
  /**
   * Refund deployment credits when the queue enqueue step fails before any
   * cloud resources (GitHub repo, Koyeb app, SaaS CD upstream) have been created.
   *
   * This is the mirror of chargeForDeployment for the pre-queue failure window.
   * The method re-throws on internal failure so the caller's .catch() receives
   * the error and can log it with full context — but callers MUST NOT let this
   * propagate to the user; they should swallow it after logging.
   */
  async refundDeploymentCredits(params: {
    userId: string;
    projectId: string;
    creditsToRefund: number;
  }): Promise<void> {
    const { userId, projectId, creditsToRefund } = params;

    if (creditsToRefund <= 0) return;

    this.logger.log(
      `Refunding ${creditsToRefund} credits for user ${userId} (project: ${projectId}) — pre-queue failure`,
    );

    await this.prisma.$transaction(async (tx) => {
      const user = await tx.users.findUnique({
        where: { id: userId },
        include: { Credits: true, TopUp: true, Subscription: true },
      });

      if (!user) {
        throw new Error(`User not found during credit refund: ${userId}`);
      }
      ...
      switch (user.currentPlan) {
        case 'FREE': {
          ...
          if (!c)
            throw new Error(
              `No credits record found for FREE user during refund: ${userId}`,
            );
          const newUsage = Math.max(0, (c.creditUsage || 0) - creditsToRefund);
          await tx.credits.update({
            where: { id: c.id },
            data: {
              creditUsage: newUsage,
              // Restore ACTIVE if a concurrent charge capped the status to DONE
              // and our refund brings usage back below the limit.
              ...(c.status === 'DONE' && newUsage < c.availableCredits
                ? { status: 'ACTIVE' }
                : {}),
            },
          });
        ...
        case 'TOP_UP': {
          ...
          if (!t)
            throw new Error(
              `No top-up record found for TOP_UP user during refund: ${userId}`,
            );
          const newUsage = Math.max(0, t.creditUsage - creditsToRefund);
          await tx.topUp.update({
            where: { id: t.id },
            data: {
              creditUsage: newUsage,
              ...(t.status === 'DONE' && newUsage < t.totalCredits
                ? { status: 'ACTIVE' }
                : {}),
            },
          });
        ...
        case 'SUBSCRIPTION': {
          ...
          if (!s)
            throw new Error(
              `No subscription record found for SUBSCRIPTION user during refund: ${userId}`,
            );
          const newUsage = Math.max(0, s.creditUsage - creditsToRefund);
        ...
        default:
          throw new Error(
            `Unknown payment plan during credit refund: ${user.currentPlan} (user: ${userId})`,
          );
      }
    });

    this.logger.log(
      `✅ Refunded ${creditsToRefund} credits for user ${userId} (project: ${projectId})`,
    );
  }
   ...
   * Also used for MemeGPT tool charging with appropriate toolType.
  ...
  private async sendToOpenMeter(params: {
    ...
    try {
      ...
      const openMeterPayload = {
        specversion: '1.0',
        type: params.type, // Event type: meme-gen-sandbox, meme-gen-ai-model
